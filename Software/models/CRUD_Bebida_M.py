...
def obtener_bebidas_por_id(id_bebidas): # Recibe una lista de ids
    bebidas = []
    for id in id_bebidas:
        bebidas.append(obtener_bebida_por_id(id))
    return bebidas
...
def agregar_bebida(n, desc, cos, vent=0):
    if estatus_conexion():
        new_row = Bebida(nombre=n, descripcion=desc, costo=abs(float(cos)), ventas=vent) # type: ignore
        session = db.session
        try:
            session.add(new_row)
            # Confirmar la transacción
