...
from controllers.__init__ import obtener_bebidas, eliminar_bebida, actualizar_bebida, obtener_bebida_por_id, session
...
def create():
    if estatus_conexion:
        if request.method == "POST":
            ...
            return render_template("Bebida/create_bebida.html")
    
@readBluePrint.route("/read", methods=["GET","POST"]) # type: ignore
def read():
    if estatus_conexion:
        productos = obtener_bebidas()
        return render_template("Bebida/index_bebidas.html", bebidas=productos)
...
def update(id):
    ...
    if estatus_conexion:
        if request.method == "POST":
            ...
            if row is not None:
                return render_template("Bebida/edit_bebida.html", id_p=id, nombre_p=row.nombre, descripcion_p=row.descripcion, costo_p = row.costo, ventas_p=row.ventas)
            return render_template("Bebida/edit_bebida.html", id_p=id)
